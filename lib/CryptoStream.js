...
CryptoStream.prototype._transform = function (chunk, encoding, callback) {
    try {
        ...
        if (!this.encrypt && Buffer.isBuffer(chunk)) {
...
CryptoStream.prototype.getCipher = function (callback) {
    ...
    if (this.encrypt) {
        ...
        // setting autopadding to be true.
     ...
     *
     * After the data is piped to the cryptostream decrypts the buffer by the library internally,
     * then this function reads the chunks and concatinates to a buffer.
     *
     * We also additionally check, if the chunk that is obtained is not null before adding Buffer.
     *
     */

    cipher.on('readable', function () {
        //Dont read here twice!!!!!
        try {
