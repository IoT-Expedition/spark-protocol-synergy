...
SparkCore.prototype = extend(ISparkCore.prototype, EventEmitter.prototype, {
    ...
    startupProtocol: function () {
        ...
        this.socket.setKeepAlive(true, 3600 * 1000); //every 1 hr
        this.socket.setTimeout(settings.socket_time_out);
    ...
    handshake: function () {
        ...
        //This function forwards the function pointer of this.ready & this.disconnect to the handshake function.
     ...
     *
     * SynergyProtocol is binded the onCoreSynergyProtocol function.
     * The SynergyProtocol is part of the COAP message which is
     * defined in Messages.js
     * aswell as on the system-firmware in messages.cpp
     *
     */
    /*
     After successful handhsake, ready function is called.
     */
    ready: function () {
        //oh hai!
    ...
    sendMessage: function (name, params, data, onResponse, onError, requester) {
        ...
        //The token is the token for the COAP protocol, please google for more info.
        ...
        // The data "hello" is encrypted and send to the device, write function is a internal Cryptostream method. Check Cryptostream.js
        this.secureOut.write(msg, null, null);
...
//            "BYTES: " + msg.toString('hex'));
    ...
    flashCore: function (binary, sender) {
         ...
         * @description: revised by SynergyLabs @ Carnegie Mellon University (2018).
         * Changed the msg from the flasher library itself
         *
         * @type {Flasher}
         */
        ...
        flasher.startFlashBuffer(binary, this,
            function (msg) {
                ...
                that.sendApiResponse(sender, { cmd: "Event", name: "Update", message: msg });
            },
            function (msg) {
...
//                eat_message = false;
    ...
    //     var stamp = fs.readFileSync('readableTime.txt', 'utf8');
    //     var addString = fs.readFileSync('readableText.txt', 'utf8');
    //     var binVal = messages.MyToBinary(addString.length, stamp, addString);
    //     this.sendReply("myReturn", msg.getId(), binVal, msg.getToken());
    // },
